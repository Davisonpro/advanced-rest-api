...
class DbMySQLiCore extends Db
{
    ...
    public function connect()
    {
        ...
        if (\mysqli_connect_error()) {
            throw new \Exception(sprintf(Tools::displayError('Link to database cannot be established: %s'), \mysqli_connect_error()));
        }

        // UTF-8 support
        if (!$this->link->query('SET NAMES \'utf8\'')) {
            throw new \Exception(Tools::displayError('HakikaPay Fatal error: no utf-8 support. Please check your server configuration.'));
        }
